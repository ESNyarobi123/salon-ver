...
Future<void> initializeBackgroundService() async {
  ...
  await service.configure(
    androidConfiguration: AndroidConfiguration(
      ...
      initialNotificationTitle: SalonStrings.foregroundServiceTitle,
      initialNotificationContent: SalonStrings.foregroundServiceBody,
  ...
  if (androidPlugin != null) {
    ...
    await androidPlugin.createNotificationChannel(
      AndroidNotificationChannel(
        _fgChannelId,
        SalonStrings.notifForegroundChannelName,
    ...
    await androidPlugin.createNotificationChannel(
      AndroidNotificationChannel(
        _channelId,
        SalonStrings.notifIncomingChannelName,
        description: SalonStrings.notifIncomingChannelDesc,
...
Future<void> _bgCheckForCalls(ServiceInstance service) async {
  try {
    ...
    for (final item in data) {
      ...
      final type = item['type'] as String? ?? 'call_waiter';
      final tableNumber = item['table_number']?.toString();
      final displayType = SalonStrings.requestTypeLabel(type);
      final tableDisplay =
          SalonStrings.requestTableDisplay(tableNumber);
...
void _showCallNotification(String title, String body, int callId) {
  _notificationsPlugin.show(
    _callNotificationId,
    '📞 $title',
    SalonStrings.notifCallBodyLine(body),
    NotificationDetails(
      android: AndroidNotificationDetails(
        _channelId,
        SalonStrings.notifIncomingChannelName,
        channelDescription: SalonStrings.notifIncomingChannelDesc,
        importance: Importance.max,
        ...
        ticker: '${SalonStrings.notifIncomingTicker} - $title',
      ),
