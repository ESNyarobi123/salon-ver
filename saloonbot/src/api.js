...
module.exports = {
    ...
    getActiveOrder: async (restaurantId, tableNumber) => {
        ...
        // Backward compat: older Laravel only returned `data`
        if (d.success && d.data && !d.order) {
            const x = d.data;
            d.order = {
                id: x.order_id,
                ...
                waiter_name: x.waiter_name ?? null,
                items: x.items || [],
            };
        }
        return d;
    ...
    /**
     * Guest Wi‑Fi (manager API)
     * GET /api/bot/restaurant/{restaurantId}/guest-wifi
     * Response: { success, data: { wifi_ssid, wifi_password } }
     */
    getGuestWifi: async (restaurantId) => {
        try {
            const response = await api.get(`/restaurant/${restaurantId}/guest-wifi`);
            return response.data;
        } catch (error) {
            console.error('Get guest WiFi error:', error.response?.data || error.message);
            return { success: false, message: error.response?.data?.message || 'Could not load Wi‑Fi details' };
        }
    },
