...
class _PayslipScreenState extends State<PayslipScreen>
  ...
  Future<void> _loadSlips() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _slipsError = SalonStrings.payslipLoadSlipsError;
        _slipsLoading = false;
      });
    }
  }
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _historyError = SalonStrings.payslipLoadHistoryError;
        _historyLoading = false;
      });
    }
  }
                ...
                Tab(
                  child: Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 6),
                      Text(SalonStrings.payslipTabSlips),
                    ],
                  ),
                ),
                Tab(
                  child: Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 6),
                      Text(SalonStrings.payslipTabHistory),
                    ],
                  ),
                ),
              ],
  ...
  Widget _buildSalarySlipsTab() {
    ...
    if (_slips.isEmpty) {
      return _buildEmptyState(
        ...
        SalonStrings.payslipNoSlips,
        SalonStrings.payslipNoSlipsHint,
      );
    }
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      SalonStrings.payslipTotalEarnings,
                      style: GoogleFonts.poppins(
                        fontSize: 12,
                        color: Colors.white.withValues(alpha: 0.5),
                      ),
              ...
              Container(
                ...
                child: Text(
                  SalonStrings.payslipSlipCount(_slips.length),
                  style: GoogleFonts.poppins(
                    fontSize: 11,
                    fontWeight: FontWeight.w600,
  ...
  Widget _buildWorkHistoryTab() {
    ...
    if (_history.isEmpty) {
      return _buildEmptyState(
        ...
        SalonStrings.payslipNoHistory,
        SalonStrings.payslipNoHistoryHint,
      );
    }
    return RefreshIndicator(
  ...
  Widget _buildHistoryCard(WorkHistoryEntry entry) {
    ...
    final statusLabel =
        isActive ? SalonStrings.payslipActive : SalonStrings.payslipEnded;
    final typeLabel = entry.employmentType == 'temporary'
        ? SalonStrings.payslipTemporary
        : SalonStrings.payslipPermanent;
    ...
    if (entry.linkedAt != null) {
      try {
        ...
        if (entry.unlinkedAt != null) {
          ...
          dateRange += ' — ${SalonStrings.payslipPresent}';
        }
      } catch (_) {}
    }
                      ...
                      children: [
                        Text(
                          entry.restaurantName ??
                              SalonStrings.payslipUnknownRestaurant,
                          style: GoogleFonts.poppins(
                            fontSize: 15,
                            fontWeight: FontWeight.w600,
                            color: Colors.white,
              ...
              Wrap(
                ...
                children: [
                  ...
                  if (entry.linkedUntil != null &&
                    ...
                    _infoPill(
                      Icons.timer_rounded,
                      SalonStrings.payslipLinkedUntil(entry.linkedUntil!),
                    ),
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              label: const Text(SalonStrings.payslipRetry),
...
class _SlipDetailSheetState extends State<_SlipDetailSheet> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        _error = SalonStrings.payslipDetailLoadError;
        _loading = false;
      });
    }
  }
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      SalonStrings.payslipDetailTitle,
                      style: GoogleFonts.poppins(
                        fontSize: 18,
                        fontWeight: FontWeight.w700,
                        color: Colors.white,
      ...
      children: [
        ...
        GlassCard(
          child: Padding(
            ...
            child: Column(
              children: [
                _metaRow(SalonStrings.payslipMetaPeriod, d.periodLabel),
                if (d.restaurantName != null)
                  _metaRow(
                    SalonStrings.payslipMetaRestaurant,
                    d.restaurantName!,
                  ),
                if (d.waiterName != null)
                  _metaRow(SalonStrings.payslipMetaEmployee, d.waiterName!),
                if (d.waiterId != null)
                  _metaRow(SalonStrings.payslipMetaEmployeeId, d.waiterId!),
                if (d.paidAt != null) ...[
                  _metaRow(
                    SalonStrings.payslipMetaPaidOn,
                    _formatDate(d.paidAt!),
                  ),
                ],
              ],
            ),
        ...
        // Earnings
        _sectionTitle(SalonStrings.payslipSectionEarnings, const Color(0xFF10B981)),
        GlassCard(
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
              children: [
                _amountRow(SalonStrings.payslipBasicSalary, d.basicSalary, fmt),
                _amountRow(SalonStrings.payslipAllowances, d.allowances, fmt),
                const Divider(color: Colors.white12, height: 20),
                _amountRow(
                  SalonStrings.payslipGrossSalary,
                  d.grossSalary,
                  fmt,
                  isBold: true,
                  color: const Color(0xFF10B981),
        ...
        // Deductions
        _sectionTitle(SalonStrings.payslipSectionDeductions, AppTheme.alert),
        GlassCard(
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
              children: [
                _amountRow(SalonStrings.payslipPaye, d.paye, fmt, isNegative: true),
                _amountRow(SalonStrings.payslipNssf, d.nssf, fmt, isNegative: true),
                const Divider(color: Colors.white12, height: 20),
                _amountRow(
                  SalonStrings.payslipTotalDeduction,
                  d.totalDeduction,
                  fmt,
                  isBold: true,
          ...
          child: Row(
            ...
            children: [
              Column(
                ...
                children: [
                  Text(
                    SalonStrings.payslipNetPayHero,
                    style: GoogleFonts.poppins(
                      fontSize: 12,
                      fontWeight: FontWeight.w600,
          ...
          children: [
            Expanded(
              child: _actionButton(
                ...
                SalonStrings.payslipDownloadPdf,
                () => _downloadSlip(d),
              ),
            ),
            const SizedBox(width: 12),
  ...
  void _downloadSlip(SalarySlipDetail d) {
    ...
    if (downloadUrl != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(SalonStrings.payslipDownloadingSnack(d.periodLabel)),
  ...
  void _shareSlip(SalarySlipDetail d) {
    final text =
        '''
${SalonStrings.payslipDetailTitle} - ${d.periodLabel}
${d.restaurantName ?? ''}
${SalonStrings.payslipMetaEmployee}: ${d.waiterName ?? ''}

${SalonStrings.payslipBasicSalary}: Tsh ${widget.currencyFmt.format(d.basicSalary)}
${SalonStrings.payslipAllowances}: Tsh ${widget.currencyFmt.format(d.allowances)}
${SalonStrings.payslipGrossSalary}: Tsh ${widget.currencyFmt.format(d.grossSalary)}
${SalonStrings.payslipPaye}: -Tsh ${widget.currencyFmt.format(d.paye)}
${SalonStrings.payslipNssf}: -Tsh ${widget.currencyFmt.format(d.nssf)}
─────────────
${SalonStrings.payslipNetPayHero}: Tsh ${widget.currencyFmt.format(d.netPay)}
''';
